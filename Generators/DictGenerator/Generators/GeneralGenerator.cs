...
namespace DictGenerator.Generators
{
    ...
    public class GeneralGenerator : IGenerator
    {
        ...
        public string Generate()
        {
            ...
            foreach (var c in chinese)
            {
                ...
                try
                {
                    ...
                    sb.AppendLine($@"{" ".Repeat(12)}new ChineseWord {{ Pinyins = new[] {{ {pinyins.Select(pinyin => $@"""{pinyin}""").Join(", ")} }}, Simplified = ""{simplified}"", Traditional = ""{traditional}"", Tag = {tag}, Polyphonic = {(polyphonic ? "true" : "false")} }},");
                    success++;
                }
                catch
                {
